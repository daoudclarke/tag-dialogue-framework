...
//                                    .create();
    ...
    public Dialogue interpret(String message, User user, Dialogue dialogue){
        ...
        List<IntentMatch> intentMatches = getIntentMatches(intents);
    ...
    private List<IntentMatch> getIntentMatches(List<Intent> intents) {
        List<IntentMatch> intentMatches = new ArrayList<>();
        for (Intent intent : intents) {
            IntentMatch newMatch = intent.getIntentMatch(necessarySlotsPerIntent.get(intent.getName()));
            intentMatches.add(newMatch);
        }
...
//    }
     ...
     */
    private boolean isIntentNamePresent(List<Intent> intents, String intentName){
        for (Intent intent : intents) {
            if (intent.isName(intentName)) {
                return true;
            }
        }
        return false;
    ...
    private boolean isCancellationPresent(List<Intent> intents){
        return isIntentNamePresent(intents, Intent.cancel);
    }
    ...
    private boolean isCancelAutoQueryPresent(List<Intent> intents){
        return isIntentNamePresent(intents, Intent.cancelAutoQuery);
    }
